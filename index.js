...
async function doPost(request, response, pathname) {    
    ...
    switch(pathname) {
	    ...
	    case '/ranking':
	        //como o rank n recebe parametros se existir algo no data da erro
	    	if(Object.keys(data).length > 0){
	    		answer.status = 400;
	    		return answer;
	    	}
	    	//funçao q vai tratar do ranking
	    	await rank(answer)
	    		.then(res => {answer = res;})
	    		.catch(console.log);
...
async function rank(answer){
	return new Promise(resolve => {
		//leitura do ficheiro que contem os dados do ranking
		fs.readFile('ranking.json', async function(err, ranks){
			if(!err){//ficheiro existe
				let d=JSON.parse(ranks.toString());
				//console.log(d);
				//vai ordenar por ordem decerescente do nº de vitorias
				await d.sort( function (a,b){
				...
				// n tenho a certeza se isto e assim
				let r= [];
				let tam;
				if(d.length >10)
					tam= 10;
